...
export function CartPanel() {
	...
	return (
		<div className='hidden lg:flex flex-col w-96 border-l border-border bg-background h-[calc(100vh-64px)] min-h-0 fixed right-0 top-16'>
			<div className='shrink-0 p-4 border-b border-border bg-muted/30'>
				<div className='flex items-center justify-between gap-2'>
					<h2 className='font-bold gap-2 flex items-center text-lg'>
						<ShoppingCart className='h-5 w-5 shrink-0' /> Current
						Order
					</h2>
					<Button
						type='button'
			...
			<div className='flex flex-1 min-h-0 flex-col overflow-hidden border-t border-border'>
				<ScrollArea className='flex-1 min-h-0'>
					<div>
						{cart.length === 0 ? (
							<div className='p-8 text-center text-muted-foreground flex flex-col items-center justify-center min-h-[120px] gap-2'>
				...
				{paymentMethod === 'cash' && (
					<div className='space-y-2 rounded-lg border border-border bg-muted/20 p-3'>
						<label
							htmlFor='pos-cash-tendered-panel'
							className='text-xs font-medium text-muted-foreground'
						>
							Cash tendered
						</label>
						<Input
							id='pos-cash-tendered-panel'
							type='number'
							inputMode='decimal'
							min={0}
							step='0.01'
